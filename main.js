...
**Analysis Standards:** 800-1000 words comprehensive documentation, 69+ citations including timestamps and external verification, complete Obsidian integration with linking strategy, research-grade video analysis suitable for professional knowledge management and academic reference.`,providerSearchModes:{gemini:!0,perplexity:!0,tavily:!0,exa:!0},contextMemoryEnabled:!0,maxContextMessages:10,contextMemoryStrategy:"recent",configurationPresets:{QuickFactCheck:{description:"Fast, concise answers for quick verification.",modeName:"quick",provider:"perplexity",parameters:{temperature:.2,max_tokens:500,search_recency_filter:"day"}},DeepDiveAnalysis:{description:"In-depth academic research for detailed papers.",modeName:"deep",provider:"gemini",parameters:{temperature:.8,topP:.9,maxOutputTokens:4e3}},CreativeBrainstorming:{description:"Generate creative ideas and outlines.",modeName:"comprehensive",provider:"gemini",parameters:{temperature:1.2,topP:.95}}},uiPreferences:{showAdvancedSettings:!1,showParameterTooltips:!0,autoSaveSettings:!0,showPerformanceMetrics:!1,compactMode:!1},researchModeModels:{quick:"gemini-2.5-flash-lite",comprehensive:"gemini-2.5-flash",deep:"gemini-2.5-pro",reasoning:"gemini-2.5-pro",youtube:"gemini-2.5-pro"}},T="gemini-chat-view",L=class extends p.Plugin{getCurrentResearchMode(){var e,i;try{let t=(i=(e=this.app.workspace.getLeavesOfType(T))==null?void 0:e[0])==null?void 0:i.view,s=t==null?void 0:t.currentResearchMode;return s&&P.validateResearchMode(s.id)?s.id:"comprehensive"}catch(t){return w.getInstance().warn("Failed to get current research mode, using default",t),"comprehensive"}}async onload(){let e=w.getInstance(),i=S.getInstance(),t="plugin-load";try{e.info("Plugin loading started"),i.startTimer(t),await this.loadSettings(),this.injectEnhancedCSS(),this.registerView(T,r=>new _(r,this)),this.addRibbonIcon("message-circle","Open AI Web Search Chat",()=>{this.activateView()}),this.addCommand({id:"gemini-web-search-selection",name:"Search Web for Selected Text",editorCallback:async(r,c)=>{let o=r.getSelection();o?await this.searchWeb(o,"replace"):new p.Notice("Please select some text first")}}),this.addCommand({id:"gemini-web-search-append",name:"Search Web for Selected Text (Append)",editorCallback:async(r,c)=>{let o=r.getSelection();o?await this.searchWeb(o,"append"):new p.Notice("Please select some text first")}}),this.addCommand({id:"open-ai-chat",name:"Open AI Web Search Chat",callback:()=>{this.activateView()}}),this.addCommand({id:"gemini-web-search-prompt",name:"AI Web Search: Custom query",editorCallback:(r,c)=>{this.promptForCustomSearch(r)}}),this.addCommand({id:"gemini-new-chat",name:"AI Web Search: Start New Chat",callback:()=>{this.activateView().then(()=>{var c;let r=(c=this.app.workspace.getLeavesOfType(T)[0])==null?void 0:c.view;r&&r.clearChat()})}}),this.addCommand({id:"test-perplexity-api",name:"AI Web Search: Test Perplexity API",callback:async()=>{await this.testPerplexityAPI()}}),this.addRibbonIcon("search","AI Web Search",()=>{this.showSearchModal()}),this.addSettingTab(new K(this.app,this));let s=i.endTimer(t);e.info(`Plugin loaded successfully in ${s}ms`)}catch(s){e.error("Plugin loading failed",s),new p.Notice(`AI Web Search Plugin failed to load: ${s}`)}}async loadSettings(){this.settings=Object.assign({},O,await this.loadData())}async saveSettings(){await this.saveData(this.settings)}showSearchModal(){new N(this.app,this).open()}async activateView(){let{workspace:e}=this.app,i=null,t=e.getLeavesOfType(T);t.length>0?i=t[0]:(i=e.getRightLeaf(!1),i&&await i.setViewState({type:T,active:!0})),i&&e.revealLeaf(i)}async performWebSearch(e){let i=w.getInstance(),t=S.getInstance(),s=`web-search-${this.settings.provider}-${Date.now()}`;try{if(t.startTimer(s),i.debug("Starting web search",{provider:this.settings.provider,query:P.sanitizeInput(e).substring(0,100)+"..."}),!P.validateQuery(e))throw new Error("Invalid search query provided");if(!P.validateProvider(this.settings.provider))throw new Error(`Invalid provider: ${this.settings.provider}`);let r;switch(this.settings.provider){case"gemini":r=await this.searchWithGemini(e);break;case"perplexity":r=await this.searchWithPerplexity(e);break;case"tavily":r=await this.searchWithTavily(e);break;case"exa":r=await this.searchWithExa(e);break;default:throw new Error(`Invalid provider: ${this.settings.provider}`)}let c=t.endTimer(s);return t.logMetrics("web-search",c,{provider:this.settings.provider,queryLength:e.length,resultLength:r.length}),r}catch(r){t.endTimer(s),i.error("Web search failed",r);let c=r instanceof Error?r.message:String(r);return`Search failed for: "${e}"

Error: ${c}

Please check your API configuration and try again.`}}async performGeminiChat(e,i){var r,c,o,n;if(!this.settings.geminiApiKey)throw new Error("Gemini API key not configured");let t=w.getInstance();t.debug("Starting Gemini chat",{query:e,historyLength:i.length});let s=[];i.forEach(a=>{s.push({role:a.role==="user"?"user":"model",parts:[{text:a.content}]})}),s.push({role:"user",parts:[{text:e}]});try{let l=(await(0,p.requestUrl)({url:`https://generativelanguage.googleapis.com/v1beta/models/${this.settings.geminiModel}:generateContent`,method:"POST",headers:{"Content-Type":"application/json","x-goog-api-key":this.settings.geminiApiKey},body:JSON.stringify({contents:s,generationConfig:{temperature:.7,maxOutputTokens:2048,topP:.8,topK:40}})})).json;if(l.candidates&&((n=(o=(c=(r=l.candidates[0])==null?void 0:r.content)==null?void 0:c.parts)==null?void 0:o[0])!=null&&n.text))return l.candidates[0].content.parts[0].text;throw new Error("No valid response from Gemini API")}catch(a){t.error("Gemini chat failed",a);let l=a instanceof Error?a.message:String(a);throw new Error(`Gemini chat failed: ${l}`)}}async performPerplexityChat(e,i){var c,o;if(!this.settings.perplexityApiKey)throw new Error("Perplexity API key not configured");let t=w.getInstance();t.debug("Starting Perplexity chat",{query:e,historyLength:i.length});let s=[];i.forEach(n=>{s.push({role:n.role,content:n.content})}),s.push({role:"user",content:e});let r={model:this.settings.perplexityModel,messages:s,temperature:this.settings.perplexityTemperature,max_tokens:this.settings.perplexityMaxTokens,top_p:this.settings.perplexityTopP,top_k:this.settings.perplexityTopK,frequency_penalty:this.settings.perplexityFrequencyPenalty,presence_penalty:this.settings.perplexityPresencePenalty};try{let a=(await(0,p.requestUrl)({url:"https://api.perplexity.ai/chat/completions",method:"POST",headers:{Authorization:`Bearer ${this.settings.perplexityApiKey}`,"Content-Type":"application/json"},body:JSON.stringify(r)})).json;if(a.choices&&((o=(c=a.choices[0])==null?void 0:c.message)!=null&&o.content))return a.choices[0].message.content;throw new Error("No valid response from Perplexity API")}catch(n){t.error("Perplexity chat failed",n);let a=n instanceof Error?n.message:String(n);throw new Error(`Perplexity chat failed: ${a}`)}}async searchWithGemini(e){var r,c,o;let i=w.getInstance(),t=S.getInstance(),s=`gemini-search-${Date.now()}`;try{if(t.startTimer(s),i.debug("Starting Gemini search",{queryLength:e.length}),!P.validateApiKey(this.settings.geminiApiKey))throw new Error("Gemini API key not configured or invalid");let n=(r=this.app.workspace.getLeavesOfType(T)[0])==null?void 0:r.view,a=n==null?void 0:n.currentResearchMode,l=this.settings.researchModeConfigs.comprehensive.geminiParams;if(a&&P.validateResearchMode(a.id))switch(a.id){case"quick":l=this.settings.researchModeConfigs.quick.geminiParams;break;case"comprehensive":l=this.settings.researchModeConfigs.comprehensive.geminiParams;break;case"deep":l=this.settings.researchModeConfigs.deep.geminiParams;break;case"reasoning":l=this.settings.researchModeConfigs.reasoning.geminiParams;break;case"youtube":l=this.settings.researchModeConfigs.youtube.geminiParams;break}let d=e;if(this.settings.enableCustomPrompts&&a)switch(a.id){case"quick":d=this.settings.quickPrompt.replace("{query}",e);break;case"comprehensive":d=this.settings.comprehensivePrompt.replace("{query}",e);break;case"deep":d=this.settings.deepPrompt.replace("{query}",e);break;case"reasoning":d=this.settings.reasoningPrompt.replace("{query}",e);break;case"youtube":d=this.settings.youtubePrompt.replace("{query}",e).replace("{url}",e);break;default:d=this.settings.comprehensivePrompt.replace("{query}",e)}else if(a)switch(a.id){case"quick":d=`### Task: Provide Quick and Accurate Response
...
- Include actionable insights and practical applications`;break;default:d=`Please provide comprehensive analysis on: "${e}" with accurate information and reliable sources.`}let u=`https://generativelanguage.googleapis.com/v1beta/models/${this.settings.geminiModel}:generateContent?key=${this.settings.geminiApiKey}`,m;if((a==null?void 0:a.id)==="youtube"&&(n!=null&&n.getVideoContext())){let k=n.getVideoContext();if(!k)return"Error: No video context available";let A=k.url;m={contents:[{parts:[{text:n.isValidYouTubeUrl(e)?d:`Based on the YouTube video at ${A}, please answer this question: "${e}"
...
- Include timestamps or specific examples when relevant`},{fileData:{mimeType:"video/*",fileUri:A}}]}],generationConfig:{temperature:l.temperature,topP:l.topP,topK:l.topK,maxOutputTokens:l.maxOutputTokens}}}else m={contents:[{parts:[{text:d}]}],tools:[{google_search:{}}],generationConfig:{temperature:l.temperature,topP:l.topP,topK:l.topK,maxOutputTokens:l.maxOutputTokens}};let y=(c=(await(0,p.requestUrl)({url:u,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(m)})).json.candidates)==null?void 0:c[0];if(!y)throw new Error("No response from Gemini");let C=y.content.parts[0].text;if((o=y.groundingMetadata)!=null&&o.groundingChunks){C+=`

--- 
**Sources:**
`;let k=new Set;y.groundingMetadata.groundingChunks.forEach(A=>{var D;(D=A.web)!=null&&D.uri&&k.add(`- [${A.web.title||A.web.uri}](${A.web.uri})`)}),C+=Array.from(k).join(`
`)}let E=t.endTimer(s);return t.logMetrics("gemini-search",E,{queryLength:e.length,resultLength:C.length,researchMode:a==null?void 0:a.id}),C}catch(n){t.endTimer(s),i.error("Gemini search failed",n);let a=n instanceof Error?n.message:String(n);throw new Error(`Gemini search failed: ${a}`)}}async searchWeb(e,i){try{let t;switch(this.settings.provider){case"gemini":t=await this.searchWithGemini(e);break;case"perplexity":t=await this.searchWithPerplexity(e);break;case"tavily":t=await this.searchWithTavily(e);break;case"exa":t=await this.searchWithExa(e);break;default:throw new Error(`Unknown provider: ${this.settings.provider}`)}this.insertResult(t,i)}catch(t){new p.Notice(`Search failed: ${t.message}`)}}async searchWithPerplexity(e){var r,c;if(!this.settings.perplexityApiKey)throw new Error("Perplexity API key not configured");if(!this.settings.perplexityApiKey.startsWith("pplx-"))throw new Error('Invalid Perplexity API key format. Must start with "pplx-"');let i=w.getInstance(),t=S.getInstance(),s=`perplexity-search-${Date.now()}`;try{t.startTimer(s),i.debug("Starting Perplexity search",{query:e});let o=this.getCurrentResearchMode(),n,a;o==="youtube"?(n=this.settings.researchModeConfigs.comprehensive.perplexityParams,a=this.settings.researchModeConfigs.comprehensive.perplexityModel):(n=this.settings.researchModeConfigs[o].perplexityParams,a=this.settings.researchModeConfigs[o].perplexityModel);let l={"llama-3.1-sonar-small-128k-online":"sonar","llama-3.1-sonar-large-128k-online":"sonar-pro","llama-3.1-sonar-huge-128k-online":"sonar-pro","llama-3.1-70b-instruct":"sonar-reasoning","llama-3.1-8b-instruct":"sonar"};l[a]&&(i.info(`Migrating old model ${a} to ${l[a]}`),a=l[a],o==="youtube"?this.settings.researchModeConfigs.comprehensive.perplexityModel=a:this.settings.researchModeConfigs[o].perplexityModel=a,await this.saveSettings()),["sonar","sonar-pro","sonar-reasoning","sonar-reasoning-pro","sonar-deep-research"].includes(a)||(i.warn(`Invalid model ${a}, falling back to sonar`),a="sonar");let u={model:a,messages:[{role:"user",content:e}],max_tokens:n.max_tokens||1e3,temperature:n.temperature||.7,return_citations:!0,return_related_questions:!1,return_images:!1};i.debug("Perplexity API request details",{model:a,currentMode:o,params:u,apiKeyPrefix:this.settings.perplexityApiKey.substring(0,8)+"..."}),Object.keys(u).forEach(C=>{u[C]===void 0&&delete u[C]}),i.debug("Perplexity API request",{model:a,params:u});let m=await(0,p.requestUrl)({url:"https://api.perplexity.ai/chat/completions",method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${this.settings.perplexityApiKey}`},body:JSON.stringify(u)}),f=t.endTimer(s);if(t.logMetrics("Perplexity Search",f,{queryLength:e.length,model:a,maxTokens:n.max_tokens}),m.status!==200)throw i.error("Perplexity API error",{status:m.status,statusText:m.text,headers:m.headers,requestBody:u}),new Error(`Perplexity search failed: Request failed, status ${m.status}. Response: ${m.text}`);let g=m.json;i.debug("Perplexity search results received",{choices:((r=g.choices)==null?void 0:r.length)||0,model:g.model});let y=`# Perplexity Search Results

**Query:** ${e}

`;if(y+=`**Model:** ${g.model||a}

`,g.choices&&g.choices.length>0){let C=g.choices[0];C.message&&C.message.content&&(y+=`## AI Response

${C.message.content}

`),C.citations&&C.citations.length>0&&(y+=`## Sources

`,C.citations.forEach((E,k)=>{y+=`${k+1}. [${E.title||E.url}](${E.url})
`}),y+=`
`),C.related_questions&&C.related_questions.length>0&&(y+=`## Related Questions

`,C.related_questions.forEach(E=>{y+=`- ${E}
`}),y+=`
`)}return g.usage&&(y+=`## Usage Statistics

`,y+=`- **Prompt Tokens:** ${g.usage.prompt_tokens||"N/A"}
`,y+=`- **Completion Tokens:** ${g.usage.completion_tokens||"N/A"}
`,y+=`- **Total Tokens:** ${g.usage.total_tokens||"N/A"}

`),y+=`
---
*Search performed using Perplexity API (${a}) on ${new Date().toISOString()}*
`,i.info("Perplexity search completed successfully",{duration:f,model:g.model,tokensUsed:((c=g.usage)==null?void 0:c.total_tokens)||0}),y}catch(o){let n=t.endTimer(s);throw i.error("Perplexity search failed",o),t.logMetrics("Perplexity Search (Failed)",n,{error:o.message,queryLength:e.length}),new Error(`Perplexity search failed: ${o.message}`)}}async searchWithTavily(e){var r,c;if(!this.settings.tavilyApiKey)throw new Error("Tavily API key not configured");let i=w.getInstance(),t=S.getInstance(),s=`tavily-search-${Date.now()}`;try{t.startTimer(s),i.debug("Starting Tavily search",{query:e});let o={api_key:this.settings.tavilyApiKey,query:e,search_depth:this.settings.tavilySearchDepth,include_answer:this.settings.tavilyIncludeAnswer,include_images:this.settings.tavilyIncludeImages,include_raw_content:this.settings.tavilyIncludeRawContent,max_results:this.settings.tavilyMaxResults,include_domains:this.settings.tavilyIncludeDomains?this.settings.tavilyIncludeDomains.split(",").map(u=>u.trim()):void 0,exclude_domains:this.settings.tavilyExcludeDomains?this.settings.tavilyExcludeDomains.split(",").map(u=>u.trim()):void 0,topic:this.settings.tavilyTopic,days:this.settings.tavilyDays};Object.keys(o).forEach(u=>{o[u]===void 0&&delete o[u]}),i.debug("Tavily API request body",o);let n=await(0,p.requestUrl)({url:"https://api.tavily.com/search",method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(o)}),a=t.endTimer(s);if(t.logMetrics("Tavily Search",a,{queryLength:e.length,maxResults:this.settings.tavilyMaxResults,searchDepth:this.settings.tavilySearchDepth}),n.status!==200)throw new Error(`Tavily API error: ${n.status} - ${n.text}`);let l=n.json;i.debug("Tavily search results received",{resultsCount:((r=l.results)==null?void 0:r.length)||0,hasAnswer:!!l.answer});let d=`# Tavily Search Results
...
`,l.results.forEach((u,m)=>{d+=`### ${m+1}. ${u.title}

`,u.url&&(d+=`**URL:** [${u.url}](${u.url})

`),u.content&&(d+=`**Content:** ${u.content}

`),u.score&&(d+=`**Relevance Score:** ${(u.score*100).toFixed(1)}%

`),d+=`---

`})),this.settings.tavilyIncludeImages&&l.images&&l.images.length>0&&(d+=`## Related Images

`,l.images.forEach((u,m)=>{u.url&&(d+=`![Image ${m+1}](${u.url})

`)})),d+=`
---
*Search performed using Tavily API on ${new Date().toISOString()}*
`,i.info("Tavily search completed successfully",{resultsCount:((c=l.results)==null?void 0:c.length)||0,duration:a}),d}catch(o){let n=t.endTimer(s);throw i.error("Tavily search failed",o),t.logMetrics("Tavily Search (Failed)",n,{error:o.message,queryLength:e.length}),new Error(`Tavily search failed: ${o.message}`)}}async searchWithExa(e){var r,c,o;if(!this.settings.exaApiKey)throw new Error("Exa API key not configured");let i=w.getInstance(),t=S.getInstance(),s=`exa-search-${Date.now()}`;try{t.startTimer(s),i.debug("Starting Exa search",{query:e});let n={query:e,type:this.settings.exaSearchType,numResults:this.settings.exaNumResults,includeDomains:this.settings.exaIncludeDomains?this.settings.exaIncludeDomains.split(",").map(m=>m.trim()):void 0,excludeDomains:this.settings.exaExcludeDomains?this.settings.exaExcludeDomains.split(",").map(m=>m.trim()):void 0,startCrawlDate:this.settings.exaStartCrawlDate||void 0,endCrawlDate:this.settings.exaEndCrawlDate||void 0,startPublishedDate:this.settings.exaStartPublishedDate||void 0,endPublishedDate:this.settings.exaEndPublishedDate||void 0,category:this.settings.exaCategory||void 0,useAutoprompt:this.settings.exaUseAutoprompt,contents:{text:this.settings.exaIncludeText,highlights:this.settings.exaIncludeHighlights}};Object.keys(n).forEach(m=>{n[m]===void 0&&delete n[m]}),n.contents&&(Object.keys(n.contents).forEach(m=>{n.contents[m]===void 0&&delete n.contents[m]}),Object.keys(n.contents).length===0&&delete n.contents),i.debug("Exa API request body",n);let a=await(0,p.requestUrl)({url:"https://api.exa.ai/search",method:"POST",headers:{"Content-Type":"application/json","x-api-key":this.settings.exaApiKey},body:JSON.stringify(n)}),l=t.endTimer(s);if(t.logMetrics("Exa Search",l,{queryLength:e.length,numResults:this.settings.exaParams.numResults,searchType:this.settings.exaParams.type}),a.status!==200)throw new Error(`Exa API error: ${a.status} - ${a.text}`);let d=a.json;i.debug("Exa search results received",{resultsCount:((r=d.results)==null?void 0:r.length)||0,searchType:d.resolvedSearchType});let u=`# Exa Search Results

**Query:** ${e}

`;return d.resolvedSearchType&&(u+=`**Search Type:** ${d.resolvedSearchType}

`),d.context&&(u+=`## AI Context Summary

${d.context}

`),d.results&&d.results.length>0&&(u+=`## Search Results

`,d.results.forEach((m,f)=>{if(u+=`### ${f+1}. ${m.title}

`,m.url&&(u+=`**URL:** [${m.url}](${m.url})

`),m.author&&(u+=`**Author:** ${m.author}

`),m.publishedDate){let g=new Date(m.publishedDate).toLocaleDateString();u+=`**Published:** ${g}

`}m.score&&(u+=`**Relevance Score:** ${(m.score*100).toFixed(1)}%

`),m.text&&(u+=`**Content:**
${m.text}

`),m.highlights&&m.highlights.length>0&&(u+=`**Key Highlights:**
`,m.highlights.forEach(g=>{u+=`- ${g}
`}),u+=`
`),m.summary&&(u+=`**Summary:** ${m.summary}

`),m.subpages&&m.subpages.length>0&&(u+=`**Related Pages:**
`,m.subpages.slice(0,3).forEach(g=>{u+=`- [${g.title}](${g.url})
`}),u+=`
`),u+=`---

`})),d.costDollars&&(u+=`## Search Cost

`,u+=`**Total Cost:** $${d.costDollars.total}

`),u+=`
---
*Search performed using Exa API on ${new Date().toISOString()}*
`,i.info("Exa search completed successfully",{resultsCount:((c=d.results)==null?void 0:c.length)||0,duration:l,cost:((o=d.costDollars)==null?void 0:o.total)||0}),u}catch(n){let a=t.endTimer(s);throw i.error("Exa search failed",n),t.logMetrics("Exa Search (Failed)",a,{error:n.message,queryLength:e.length}),new Error(`Exa search failed: ${n.message}`)}}insertResult(e,i){let t=this.app.workspace.getActiveViewOfType(p.MarkdownView);if(!t){new p.Notice("No active markdown view");return}let s=t.editor;if(i==="replace")s.replaceSelection(e);else{let r=s.getCursor();s.replaceRange(`
...
`+e,r)}}promptForCustomSearch(e){let i=prompt("Enter your search query:");i&&this.searchWeb(i,"append")}async testPerplexityAPI(){try{let e=await this.searchWithPerplexity("test query");new p.Notice("Perplexity API test successful"),console.log("Perplexity test result:",e)}catch(e){new p.Notice(`Perplexity API test failed: ${e.message}`),console.error("Perplexity test error:",e)}}injectEnhancedCSS(){let e=`
		...
		`,i=document.createElement("style");i.textContent=e,document.head.appendChild(i)}},_=class extends p.ItemView{constructor(e,i){super(e);this.currentVideoContext=null;this.plugin=i}getViewType(){return T}getDisplayText(){return"AI Web Search Chat"}getIcon(){return"message-circle"}async onOpen(){let e=this.containerEl.children[1];e.empty(),e.addClass("gemini-chat-container");let i=e.createEl("div",{cls:"gemini-chat-header"}),t=i.createEl("div",{cls:"title-container"});t.createEl("h3",{text:"AI Web Search Chat"});let s=t.createEl("button",{cls:"new-chat-button",title:"Start a new conversation (Ctrl/Cmd + N)",attr:{"aria-label":"Start new chat conversation",role:"button"}});s.innerHTML=`
			...
			<span>New Chat</span>
		`,s.addEventListener("click",()=>{this.clearChat()}),this.containerEl.addEventListener("keydown",f=>{(f.ctrlKey||f.metaKey)&&f.key==="n"&&!f.shiftKey&&!f.altKey&&(f.preventDefault(),this.clearChat())});let r=i.createEl("div",{cls:"provider-container"});r.createEl("span",{text:"Provider: ",cls:"provider-label"});let c=r.createEl("select",{cls:"provider-dropdown"}),o=c.createEl("option",{value:"gemini",text:`Google Gemini ${this.checkApiKey("gemini")?"\u2713":"\u26A0\uFE0F"}`}),n=c.createEl("option",{value:"perplexity",text:`Perplexity AI ${this.checkApiKey("perplexity")?"\u2713":"\u26A0\uFE0F"}`}),a=c.createEl("option",{value:"tavily",text:`Tavily Search ${this.checkApiKey("tavily")?"\u2713":"\u26A0\uFE0F"}`}),l=c.createEl("option",{value:"exa",text:`Exa AI Search ${this.checkApiKey("exa")?"\u2713":"\u26A0\uFE0F"}`});c.value=this.plugin.settings.provider;let d=i.createEl("div",{cls:"model-container"});d.createEl("span",{text:"Model: ",cls:"model-label"});let u=d.createEl("select",{cls:"model-dropdown"});this.updateModelDropdown(u,this.plugin.settings.provider),c.addEventListener("change",async f=>{var y;let g=f.target.value;this.plugin.settings.provider=g,this.updateModelDropdown(u,g),((y=this.currentResearchMode)==null?void 0:y.id)==="youtube"&&g!=="gemini"&&(new p.Notice("\u26A0\uFE0F YouTube mode requires Gemini provider"),c.value="gemini",this.plugin.settings.provider="gemini",this.updateModelDropdown(u,"gemini")),await this.plugin.saveSettings()}),u.addEventListener("change",async f=>{let g=f.target.value;await this.updateModelForCurrentMode(g)}),this.messageContainer=e.createEl("div",{cls:"gemini-chat-messages"}),this.inputContainer=e.createEl("div",{cls:"gemini-chat-input-container"}),this.createInputArea(),this.currentResearchMode={id:"comprehensive",label:"\u{1F50D} Comprehensive",description:"Balanced research with detailed analysis",model:"gemini-2.5-flash",perplexityModel:"sonar-pro",exaSearchType:"auto",exaCategory:""},this.checkApiKey(this.plugin.settings.provider)?this.addMessage("system",`Welcome! Ask me anything and I'll search the web for you using ${this.plugin.settings.provider}.`):this.addMessage("system",`\u26A0\uFE0F Welcome! Please configure your ${this.plugin.settings.provider} API key in plugin settings before starting.`)}createInputArea(){var l;this.inputContainer.empty();let e=this.inputContainer.createEl("div",{cls:"input-group"}),i=e.createEl("textarea",{cls:"gemini-chat-input",attr:{placeholder:"Ask anything...",rows:"3"}}),t=e.createEl("div",{cls:"button-group"}),s=t.createEl("button",{cls:"send-button",text:"Send"}),r=t.createEl("button",{cls:"save-button",text:"Send & Save",attr:{title:"Send query and save chat to folder"}}),c=this.inputContainer.createEl("div",{cls:"research-mode-container-bottom"});c.createEl("div",{text:"Research Mode:",cls:"research-mode-label-small"});let o=c.createEl("div",{cls:"research-mode-buttons-bottom"});[{id:"quick",label:"\u26A1 Quick",description:"Fast answers",model:"gemini-2.5-flash-lite",perplexityModel:"sonar",exaSearchType:"fast",exaCategory:""},{id:"comprehensive",label:"\u{1F50D} Comprehensive",description:"Balanced research",model:"gemini-2.5-flash",perplexityModel:"sonar-pro",exaSearchType:"auto",exaCategory:""},{id:"deep",label:"\u{1F3AF} Deep",description:"Expert analysis",model:"gemini-2.5-pro",perplexityModel:"sonar-deep-research",exaSearchType:"neural",exaCategory:"research paper"},{id:"reasoning",label:"\u{1F9E0} Reasoning",description:"Complex analysis",model:"gemini-2.5-pro",perplexityModel:"sonar-reasoning",exaSearchType:"neural",exaCategory:"research paper"},{id:"youtube",label:"\u{1F3AC} YouTube",description:"Video analysis",model:"gemini-2.5-pro",perplexityModel:"sonar-pro",exaSearchType:"auto",exaCategory:"",providerLock:"gemini",requiresUrl:!0}].forEach(d=>{let u=o.createEl("button",{cls:`research-mode-btn-small research-mode-${d.id}`,attr:{"data-mode":d.id},text:d.label});u.addEventListener("click",()=>{this.setResearchMode(d),o.querySelectorAll(".research-mode-btn-small").forEach(f=>f.removeClass("active")),u.addClass("active")})}),(l=o.querySelector('[data-mode="comprehensive"]'))==null||l.addClass("active"),s.onclick=()=>this.handleSend(i.value,!1),r.onclick=()=>this.handleSend(i.value,!1,!0);let a=()=>{i.style.height="auto",i.style.height=Math.min(i.scrollHeight,200)+"px"};i.addEventListener("input",a),i.addEventListener("keydown",d=>{d.key==="Enter"&&!d.shiftKey&&(d.preventDefault(),this.handleSend(i.value,!1))}),a()}async handleSend(e,i,t=!1){var u,m,f;if(!e.trim())return;let s=this.inputContainer.querySelector(".gemini-chat-input"),r=this.inputContainer.querySelector(".send-button"),c=this.inputContainer.querySelector(".save-button"),o=s.value;s.value="",s.disabled=!0,r.disabled=!0,c.disabled=!0,this.addMessage("user",e);let n=this.plugin.settings.provider,a=(m=(u=this.plugin.settings.providerSearchModes)==null?void 0:u[n])!=null?m:!0,l="Thinking...";a?l=`Searching the web with ${n}...`:l=`Chatting with ${n}...`;let d=this.addMessage("assistant",l,!0);if((f=this.currentResearchMode)!=null&&f.requiresUrl&&!this.isValidYouTubeUrl(e)&&!this.currentVideoContext){let g="\u274C YouTube mode requires a valid YouTube URL. Please paste a YouTube video URL first.";this.updateMessage(d,g);return}try{let g;a?g=await this.plugin.performWebSearch(e):n==="gemini"?g=await this.plugin.performGeminiChat(e,this.getChatHistory()):n==="perplexity"?g=await this.plugin.performPerplexityChat(e,this.getChatHistory()):g=await this.plugin.performWebSearch(e),this.updateMessage(d,g),i&&this.insertToActiveNote(g),t&&this.plugin.settings.chatSaveEnabled&&await this.saveToFolder()}catch(g){let y=`\u274C Error: ${g instanceof Error?g.message:String(g)}`;this.updateMessage(d,y),s.value=o}finally{s.disabled=!1,r.disabled=!1,c.disabled=!1,s.focus()}}getChatHistory(){if(!this.plugin.settings.contextMemoryEnabled)return[];let e=[];switch(this.messageContainer.querySelectorAll(".chat-message").forEach(t=>{var c;let s=t.getAttribute("data-role"),r=((c=t.querySelector(".message-content"))==null?void 0:c.textContent)||"";s&&r&&s!=="system"&&!r.includes("Searching the web...")&&!r.includes("Thinking...")&&!r.includes("Analyzing YouTube video...")&&e.push({role:s==="user"?"user":"assistant",content:r})}),this.plugin.settings.contextMemoryStrategy){case"recent":return e.slice(-this.plugin.settings.maxContextMessages);case"token-limit":let t=this.plugin.settings.maxContextMessages*100,s=0,r=[];for(let d=e.length-1;d>=0;d--){let u=Math.ceil(e[d].content.length/4);if(s+u>t)break;s+=u,r.unshift(e[d])}return r;case"summary":if(e.length<=this.plugin.settings.maxContextMessages)return e;let c=e[0],o=e.slice(-Math.floor(this.plugin.settings.maxContextMessages/2)),l={role:"system",content:`[Context Summary] Previous conversation included:
${e.slice(1,-Math.floor(this.plugin.settings.maxContextMessages/2)).map(d=>`${d.role}: ${d.content.substring(0,100)}...`).join(`
`)}`};return[c,l,...o];default:return e.slice(-this.plugin.settings.maxContextMessages)}}setResearchMode(e){this.currentResearchMode=e;let i=this.containerEl.querySelector(".model-dropdown");if(i&&this.updateModelDropdown(i,this.plugin.settings.provider),e.id==="youtube"){if(this.plugin.settings.provider!=="gemini"){this.plugin.settings.provider="gemini",this.plugin.saveSettings();let t=this.containerEl.querySelector(".provider-dropdown");t&&(t.value="gemini"),i&&this.updateModelDropdown(i,"gemini"),new p.Notice("\u{1F3AC} Switched to Gemini provider for YouTube video analysis")}this.plugin.settings.geminiModel="gemini-2.5-pro",this.plugin.saveSettings()}this.plugin.settings.provider==="gemini"?this.plugin.settings.geminiModel=e.model:this.plugin.settings.provider==="perplexity"?this.plugin.settings.perplexityModel=e.perplexityModel:this.plugin.settings.provider==="exa"&&(this.plugin.settings.exaSearchType=e.exaSearchType,this.plugin.settings.exaCategory=e.exaCategory),this.plugin.saveSettings(),this.addMessage("system",`Research mode set to ${e.label}: ${e.description}`)}updateModelDropdown(e,i){var s;e.empty(),this.getAvailableModels(i,(s=this.currentResearchMode)==null?void 0:s.id).forEach(r=>{let c=e.createEl("option",{value:r.id,text:r.displayName});this.isCurrentModel(r.id,i)&&(c.selected=!0)})}getAvailableModels(e,i){let t=[];switch(e){case"gemini":t.push({id:"gemini-2.5-pro",displayName:"Gemini 2.5 Pro (Best Quality)",compatible:["comprehensive","deep","reasoning","youtube"]},{id:"gemini-2.5-flash",displayName:"Gemini 2.5 Flash (Balanced)",compatible:["quick","comprehensive","deep"]},{id:"gemini-2.5-flash-lite",displayName:"Gemini 2.5 Flash Lite (Fastest)",compatible:["quick"]});break;case"perplexity":t.push({id:"sonar-reasoning-pro",displayName:"Sonar Reasoning Pro (Advanced)",compatible:["reasoning","deep"]},{id:"sonar-pro",displayName:"Sonar Pro (Advanced Search)",compatible:["comprehensive","deep"]},{id:"sonar",displayName:"Sonar (Standard)",compatible:["quick","comprehensive"]},{id:"sonar-deep-research",displayName:"Sonar Deep Research (Exhaustive)",compatible:["deep"]},{id:"sonar-reasoning",displayName:"Sonar Reasoning (Fast)",compatible:["reasoning"]});break;case"tavily":t.push({id:"tavily-search",displayName:"Tavily Search",compatible:["quick","comprehensive","deep"]});break;case"exa":t.push({id:"exa-neural",displayName:"Neural Search",compatible:["comprehensive","deep","reasoning"]});break}return i?t.filter(s=>s.compatible.includes(i)):t}isCurrentModel(e,i){var r;let t=(r=this.currentResearchMode)==null?void 0:r.id;if(!t)return!1;let s=this.plugin.settings.researchModeConfigs[t];switch(i){case"gemini":return s.geminiModel===e;case"perplexity":return s.perplexityModel===e;default:return!1}}async updateModelForCurrentMode(e){var o;let i=this.plugin.settings.provider,t=(o=this.currentResearchMode)==null?void 0:o.id;if(!t)return;let r=this.getAvailableModels(i,t).find(n=>n.id===e);if(!r){new p.Notice(`\u26A0\uFE0F Model ${e} not compatible with ${t} mode`);return}let c=t;switch(i){case"gemini":this.plugin.settings.researchModeConfigs[c].geminiModel=e;break;case"perplexity":this.plugin.settings.researchModeConfigs[c].perplexityModel=e;break}await this.plugin.saveSettings(),new p.Notice(`\u2705 Updated ${t} mode to use ${r.displayName}`)}addMessage(e,i,t=!1){let s=Date.now().toString(),r=this.messageContainer.createEl("div",{cls:`message ${e}`,attr:{"data-id":s,"data-role":e}});if(e==="user")r.createEl("div",{cls:"message-role",text:"You"});else if(e==="assistant"){let o=r.createEl("div",{cls:"message-role-header"});o.createEl("span",{cls:"message-role",text:"AI Assistant"});let n=o.createEl("button",{cls:"copy-button",text:"\u{1F4CB} Copy"});n.addEventListener("click",()=>{navigator.clipboard.writeText(i),n.textContent="\u2705 Copied!",setTimeout(()=>{n.textContent="\u{1F4CB} Copy"},2e3)})}let c=r.createEl("div",{cls:"message-content"});if(t){c.addClass("thinking");let a=c.createEl("div",{cls:"progress-container"}).createEl("div",{cls:"progress-bar"}).createEl("div",{cls:"progress-bar-fill"}),l=c.createEl("div",{cls:"thinking-text"});l.textContent=i;let d=0,u=setInterval(()=>{d+=Math.random()*15,d>=90&&(d=90,clearInterval(u)),a.style.width=`${d}%`},200);r.setAttribute("data-progress-interval",u.toString())}return c.addClass("selectable-text"),e==="assistant"&&!t?this.renderMarkdownContent(c,i):c.textContent=i,this.messageContainer.scrollTop=this.messageContainer.scrollHeight,s}updateMessage(e,i){let t=this.messageContainer.querySelector(`[data-id="${e}"]`);if(t){let s=t.getAttribute("data-progress-interval");s&&clearInterval(parseInt(s));let r=t.querySelector(".message-content");if(r){r.removeClass("thinking"),r.addClass("selectable-text");let c=r.querySelector(".progress-bar-fill");c?(c.style.width="100%",setTimeout(()=>{let n=r.querySelector(".progress-container");n&&n.remove();let a=r.querySelector(".thinking-text");a&&a.remove(),this.renderMarkdownContent(r,i),this.messageContainer.scrollTop=this.messageContainer.scrollHeight},300)):(this.renderMarkdownContent(r,i),this.messageContainer.scrollTop=this.messageContainer.scrollHeight);let o=t.querySelector(".copy-button");o&&(o.onclick=()=>{navigator.clipboard.writeText(i),o.textContent="\u2705 Copied!",setTimeout(()=>{o.textContent="\u{1F4CB} Copy"},2e3)})}}}insertToActiveNote(e){let i=this.app.workspace.getActiveViewOfType(p.MarkdownView);if(i){let t=i.editor,s=t.getCursor();t.replaceRange(`

${e}
`,s),new p.Notice("Response inserted to note")}else new p.Notice("No active note to insert into")}clearChat(){if(!(this.messageContainer&&this.messageContainer.children.length>1&&!confirm("Are you sure you want to start a new chat? This will clear the current conversation.")))if(this.messageContainer)this.messageContainer.addClass("clearing"),setTimeout(()=>{this.messageContainer.empty(),this.messageContainer.removeClass("clearing"),this.currentResearchMode={id:"comprehensive",label:"\u{1F50D} Comprehensive",description:"Balanced research with detailed analysis",model:"gemini-2.5-flash",perplexityModel:"sonar-pro",exaSearchType:"auto",exaCategory:""},this.containerEl.querySelectorAll(".research-mode-btn-small").forEach(r=>{r.removeClass("active"),r.getAttribute("data-mode")==="comprehensive"&&r.addClass("active")});let s=this.checkApiKey(this.plugin.settings.provider)?`\u{1F195} New conversation started! Ask me anything and I'll search the web for you using ${this.plugin.settings.provider}.`:`\u26A0\uFE0F New conversation started! Please configure your ${this.plugin.settings.provider} API key in plugin settings before starting.`;this.addMessage("system",s),setTimeout(()=>{let r=this.containerEl.querySelector(".gemini-chat-input");r&&(r.focus(),r.placeholder="Ask anything to start your new conversation...",setTimeout(()=>{r.placeholder="Ask anything..."},3e3))},100)},200);else{let i=this.containerEl.querySelector(".gemini-chat-input");i&&i.focus()}}checkApiKey(e){switch(e){case"gemini":return!!this.plugin.settings.geminiApiKey;case"perplexity":return!!this.plugin.settings.perplexityApiKey;case"tavily":return!!this.plugin.settings.tavilyApiKey;case"exa":return!!this.plugin.settings.exaApiKey;default:return!1}}renderMarkdownContent(e,i){e.empty();let t=i.split(`
`),s=e,r=!1,c=[];t.forEach(o=>{if(o.startsWith("```"))if(r){let a=e.createEl("pre").createEl("code");a.textContent=c.join(`
`),c=[],r=!1,s=e}else r=!0;else if(r)c.push(o);else if(o.startsWith("### ")){let n=e.createEl("h3");n.textContent=o.replace("### ",""),s=e}else if(o.startsWith("## ")){let n=e.createEl("h2");n.textContent=o.replace("## ",""),s=e}else if(o.startsWith("# ")){let n=e.createEl("h1");n.textContent=o.replace("# ",""),s=e}else if(o.startsWith("- ")||o.startsWith("* ")){s.tagName!=="UL"&&(s=e.createEl("ul"));let n=s.createEl("li");this.parseInlineMarkdown(n,o.replace(/^[*-] /,""))}else if(o.startsWith("**Sources:**")||o.startsWith("--- ")){if(o.startsWith("--- "))e.createEl("hr");else{let n=e.createEl("h4");n.textContent="Sources:"}s=e}else if(o.trim()==="")e.createEl("br"),s=e;else{s.tagName==="UL"&&(s=e);let n=e.createEl("p");this.parseInlineMarkdown(n,o)}})}parseInlineMarkdown(e,i){let t=i;t=t.replace(/\*\*(.*?)\*\*/g,"<strong>$1</strong>"),t=t.replace(/\*(.*?)\*/g,"<em>$1</em>"),t=t.replace(/`(.*?)`/g,"<code>$1</code>"),t=t.replace(/\[([^\]]+)\]\(([^)]+)\)/g,'<a href="$2" target="_blank" rel="noopener noreferrer" class="external-link">$1 \u{1F517}</a>'),t=t.replace(/(https?:\/\/[^\s<>"{}|\\^`[\]]+)/g,'<a href="$1" target="_blank" rel="noopener noreferrer" class="external-link">$1 \u{1F517}</a>'),t=t.replace(/\[(\d+)\]/g,`<a href="#citation-$1" class="citation-link" onclick="this.closest('.message-content').querySelector('h4:contains(Sources), strong:contains(Sources)')?.scrollIntoView({behavior: 'smooth', block: 'center'})">[$1]</a>`),e.innerHTML=t,e.querySelectorAll(".citation-link").forEach(r=>{r.addEventListener("click",c=>{c.preventDefault();let o=e.closest(".message-content");if(o){let n=Array.from(o.querySelectorAll("strong, h4")).find(a=>{var l;return(l=a.textContent)==null?void 0:l.includes("Sources")});n&&(n.scrollIntoView({behavior:"smooth",block:"center"}),n.style.backgroundColor="var(--interactive-accent)",n.style.color="white",n.style.padding="8px",n.style.borderRadius="4px",n.style.transition="all 0.3s ease",setTimeout(()=>{n.style.backgroundColor="",n.style.color="",n.style.padding=""},2e3))}})})}async saveToFolder(){try{let e=this.plugin.settings.chatFolderName,i=this.app.vault;i.getAbstractFileByPath(e)||await i.createFolder(e);let s=new Date().toISOString().slice(0,19).replace(/[T:]/g,"-"),c=(this.getFirstUserMessage()||"chat").slice(0,50).replace(/[^\w\s-]/g,"").replace(/\s+/g,"-"),o;switch(this.plugin.settings.chatNoteTemplate){case"timestamp-query":o=`${s}-${c}`;break;case"query-timestamp":o=`${c}-${s}`;break;case"query-only":o=c;break;case"counter":o=`chat-${i.getMarkdownFiles().filter(m=>m.path.startsWith(e)&&m.basename.startsWith("chat-")).length+1}`;break;default:o=`${s}-${c}`}let n=`${o}.md`,a=1;for(;i.getAbstractFileByPath(`${e}/${n}`);)n=`${o}-${a}.md`,a++;let l=this.formatChatNote(),d=`${e}/${n}`;await i.create(d,l),new p.Notice(`Chat saved to: ${d}`)}catch(e){new p.Notice(`Failed to save chat: ${e instanceof Error?e.message:String(e)}`),console.error("Save to folder error:",e)}}formatChatNote(){let e=new Date().toLocaleString(),i=this.plugin.settings.provider,t=this.currentResearchMode,s=`# AI Web Search Chat
...
`,this.messageContainer.querySelectorAll(".message").forEach(c=>{var a,l;let o=c.querySelector(".message-role"),n=c.querySelector(".message-content");if(o&&n){let d=(a=o.textContent)==null?void 0:a.trim(),u=((l=n.textContent)==null?void 0:l.trim())||"";d==="You"?s+=`## \u{1F64B} You

${u}

`:d==="AI Assistant"&&!u.includes("Searching the web...")&&(s+=`## \u{1F916} AI Assistant

${u}

`)}}),s+=`
---
*Generated by AI Web Search Plugin*`,s}getFirstUserMessage(){var i;let e=this.messageContainer.querySelectorAll(".message.user .message-content");return e.length>0&&((i=e[0].textContent)==null?void 0:i.trim())||""}isValidYouTubeUrl(e){return/^https?:\/\/(?:www\.)?(?:youtube\.com\/(?:watch\?v=|embed\/|v\/)|youtu\.be\/)([a-zA-Z0-9_-]{11})(?:\S+)?$/.test(e.trim())}extractYouTubeVideoId(e){let i=e.match(/(?:youtube\.com\/(?:watch\?v=|embed\/|v\/)|youtu\.be\/)([a-zA-Z0-9_-]{11})/);return i?i[1]:null}clearVideoContext(){this.currentVideoContext=null,this.updateVideoContextUI()}setVideoContext(e,i,t){this.currentVideoContext={url:e,videoId:i,title:t,analyzed:!0},this.updateVideoContextUI()}updateVideoContextUI(){var e;!this.videoContextContainer||!this.videoContextTitle||(this.currentVideoContext&&((e=this.currentResearchMode)==null?void 0:e.id)==="youtube"?(this.videoContextContainer.style.display="block",this.videoContextTitle.textContent=this.currentVideoContext.title||`Video ${this.currentVideoContext.videoId}`):this.videoContextContainer.style.display="none")}getVideoContext(){return this.currentVideoContext}async onClose(){}},N=class{constructor(h,e){this.app=h,this.plugin=e}open(){new p.Notice("Search modal coming soon...")}},K=class extends p.PluginSettingTab{constructor(e,i){super(e,i);this.plugin=i}display(){let{containerEl:e}=this;if(e.empty(),e.createEl("h2",{text:"AI Web Search Settings"}),new p.Setting(e).setName("AI Provider").setDesc("Choose your preferred AI provider").addDropdown(i=>i.addOption("gemini","Google Gemini").addOption("perplexity","Perplexity AI").addOption("tavily","Tavily").addOption("exa","Exa").setValue(this.plugin.settings.provider).onChange(async t=>{this.plugin.settings.provider=t,await this.plugin.saveSettings()})),new p.Setting(e).setName("Gemini API Key").setDesc("Enter your Google AI Studio API key").addText(i=>i.setPlaceholder("Enter API key").setValue(this.plugin.settings.geminiApiKey).onChange(async t=>{this.plugin.settings.geminiApiKey=t,await this.plugin.saveSettings()})),new p.Setting(e).setName("Perplexity API Key").setDesc("Enter your Perplexity API key").addText(i=>i.setPlaceholder("Enter API key").setValue(this.plugin.settings.perplexityApiKey).onChange(async t=>{this.plugin.settings.perplexityApiKey=t,await this.plugin.saveSettings()})),new p.Setting(e).setName("Tavily API Key").setDesc("Enter your Tavily API key").addText(i=>i.setPlaceholder("Enter API key").setValue(this.plugin.settings.tavilyApiKey).onChange(async t=>{this.plugin.settings.tavilyApiKey=t,await this.plugin.saveSettings()})),new p.Setting(e).setName("Exa API Key").setDesc("Enter your Exa API key").addText(i=>i.setPlaceholder("Enter API key").setValue(this.plugin.settings.exaApiKey).onChange(async t=>{this.plugin.settings.exaApiKey=t,await this.plugin.saveSettings()})),new p.Setting(e).setName("Insert Mode").setDesc("How to insert search results").addDropdown(i=>i.addOption("replace","Replace Selection").addOption("append","Append to Document").setValue(this.plugin.settings.insertMode).onChange(async t=>{this.plugin.settings.insertMode=t,await this.plugin.saveSettings()})),new p.Setting(e).setName("Max Results").setDesc("Maximum number of search results").addSlider(i=>i.setLimits(1,20,1).setValue(this.plugin.settings.maxResults).setDynamicTooltip().onChange(async t=>{this.plugin.settings.maxResults=t,await this.plugin.saveSettings()})),new p.Setting(e).setName("Include Images").setDesc("Include images in search results when supported").addToggle(i=>i.setValue(this.plugin.settings.includeImages).onChange(async t=>{this.plugin.settings.includeImages=t,await this.plugin.saveSettings()})),e.createEl("h3",{text:"Custom Prompts"}),new p.Setting(e).setName("Enable Custom Prompts").setDesc("Use your own custom prompts instead of built-in optimized prompts").addToggle(i=>i.setValue(this.plugin.settings.enableCustomPrompts).onChange(async t=>{this.plugin.settings.enableCustomPrompts=t,await this.plugin.saveSettings(),this.display()})),this.plugin.settings.enableCustomPrompts&&this.addCustomPromptsSettings(e),new p.Setting(e).setName("Show Advanced Settings").setDesc("Show provider-specific advanced configuration options").addToggle(i=>i.setValue(this.plugin.settings.uiPreferences.showAdvancedSettings).onChange(async t=>{this.plugin.settings.uiPreferences.showAdvancedSettings=t,await this.plugin.saveSettings(),this.display()})),this.plugin.settings.uiPreferences.showAdvancedSettings){let i=e.createDiv("advanced-settings-container");i.createEl("h3",{text:"Advanced Provider Settings"});let t=i.createDiv("advanced-tabs-container"),s=i.createDiv("advanced-content"),r=[{id:"gemini",label:"Gemini",method:()=>this.addGeminiAdvancedSettings(s)},{id:"perplexity",label:"Perplexity",method:()=>this.addPerplexityAdvancedSettings(s)},{id:"tavily",label:"Tavily",method:()=>this.addTavilyAdvancedSettings(s)},{id:"exa",label:"Exa",method:()=>this.addExaAdvancedSettings(s)}];r.forEach((c,o)=>{let n=t.createEl("button",{text:c.label,cls:o===0?"advanced-tab active":"advanced-tab"});n.onclick=()=>{t.querySelectorAll(".advanced-tab").forEach(a=>a.removeClass("active")),n.addClass("active"),s.empty(),c.method()}}),r[0].method()}}addGeminiAdvancedSettings(e){e.createEl("h5",{text:"Gemini Advanced Parameters"}),e.createEl("p",{text:"Configure advanced Gemini model parameters for fine-tuned responses. These settings affect all research modes.",cls:"settings-help-text"}),new p.Setting(e).setName("Gemini Model").setDesc("Choose the Gemini model variant (affects cost and capabilities)").addDropdown(t=>t.addOption("gemini-2.5-pro","Gemini 2.5 Pro (Best Quality)").addOption("gemini-2.5-flash","Gemini 2.5 Flash (Balanced)").addOption("gemini-2.5-flash-lite","Gemini 2.5 Flash Lite (Fastest)").setValue(this.plugin.settings.geminiModel||"gemini-2.5-flash").onChange(async s=>{this.plugin.settings.geminiModel=s,["quick","comprehensive","deep","reasoning","youtube"].forEach(c=>{this.plugin.settings.researchModeConfigs[c]&&(this.plugin.settings.researchModeConfigs[c].geminiModel=s)}),await this.plugin.saveSettings()})),new p.Setting(e).setName("Temperature").setDesc("Controls randomness: 0.0 = focused, 2.0 = creative (Default: 0.7)").addSlider(t=>t.setLimits(0,200,5).setValue((this.plugin.settings.researchModeConfigs.comprehensive.geminiParams.temperature||.7)*100).setDynamicTooltip().onChange(async s=>{let r=s/100;["quick","comprehensive","deep","reasoning","youtube"].forEach(o=>{var n;(n=this.plugin.settings.researchModeConfigs[o])!=null&&n.geminiParams&&(this.plugin.settings.researchModeConfigs[o].geminiParams.temperature=r)}),await this.plugin.saveSettings()})),new p.Setting(e).setName("Top P (Nucleus Sampling)").setDesc("Cumulative probability cutoff: 0.1 = conservative, 1.0 = diverse (Default: 0.8)").addSlider(t=>t.setLimits(10,100,5).setValue((this.plugin.settings.researchModeConfigs.comprehensive.geminiParams.topP||.8)*100).setDynamicTooltip().onChange(async s=>{let r=s/100;["quick","comprehensive","deep","reasoning","youtube"].forEach(o=>{var n;(n=this.plugin.settings.researchModeConfigs[o])!=null&&n.geminiParams&&(this.plugin.settings.researchModeConfigs[o].geminiParams.topP=r)}),await this.plugin.saveSettings()})),new p.Setting(e).setName("Top K").setDesc("Limits vocabulary to top K tokens: 1 = restrictive, 100 = diverse (Default: 40)").addSlider(t=>t.setLimits(1,100,1).setValue(this.plugin.settings.researchModeConfigs.comprehensive.geminiParams.topK||40).setDynamicTooltip().onChange(async s=>{["quick","comprehensive","deep","reasoning","youtube"].forEach(c=>{var o;(o=this.plugin.settings.researchModeConfigs[c])!=null&&o.geminiParams&&(this.plugin.settings.researchModeConfigs[c].geminiParams.topK=s)}),await this.plugin.saveSettings()})),new p.Setting(e).setName("Max Output Tokens").setDesc("Maximum response length: 256 = short, 8192 = very long (Default: 2048)").addSlider(t=>t.setLimits(256,8192,256).setValue(this.plugin.settings.researchModeConfigs.comprehensive.geminiParams.maxOutputTokens||2048).setDynamicTooltip().onChange(async s=>{["quick","comprehensive","deep","reasoning","youtube"].forEach(c=>{var o;(o=this.plugin.settings.researchModeConfigs[c])!=null&&o.geminiParams&&(this.plugin.settings.researchModeConfigs[c].geminiParams.maxOutputTokens=s)}),await this.plugin.saveSettings()})),e.createEl("h6",{text:"Content Safety Settings"}),e.createEl("p",{text:"Configure content filtering levels for different harm categories.",cls:"settings-help-text"}),[{key:"harassment",name:"Harassment",desc:"Malicious comments targeting identity/protected attributes"},{key:"hateSpeech",name:"Hate Speech",desc:"Content that promotes hatred toward groups"},{key:"sexuallyExplicit",name:"Sexually Explicit",desc:"Contains sexual or erotic content"},{key:"dangerousContent",name:"Dangerous Content",desc:"Promotes harmful or illegal activities"}].forEach(t=>{new p.Setting(e).setName(t.name).setDesc(t.desc).addDropdown(s=>s.addOption("BLOCK_NONE","None - Allow all content").addOption("BLOCK_ONLY_HIGH","High - Block only high-confidence harmful content").addOption("BLOCK_MEDIUM_AND_ABOVE","Medium+ - Block medium and high harmful content (Default)").addOption("BLOCK_LOW_AND_ABOVE","Low+ - Block low, medium, and high harmful content").setValue(this.plugin.settings.researchModeConfigs.comprehensive.geminiSafety[t.key]||"BLOCK_MEDIUM_AND_ABOVE").onChange(async r=>{["quick","comprehensive","deep","reasoning","youtube"].forEach(o=>{var n;(n=this.plugin.settings.researchModeConfigs[o])!=null&&n.geminiSafety&&(this.plugin.settings.researchModeConfigs[o].geminiSafety[t.key]=r)}),await this.plugin.saveSettings()}))})}addPerplexityAdvancedSettings(e){e.createEl("h5",{text:"Perplexity Advanced Parameters"}),e.createEl("p",{text:"Control Perplexity search depth and response characteristics.",cls:"settings-help-text"}),new p.Setting(e).setName("Perplexity Model").setDesc("Choose Perplexity AI model").addDropdown(b=>{b.addOption("sonar-reasoning-pro","Sonar Reasoning Pro (Advanced)").addOption("sonar-pro","Sonar Pro (Advanced Search)").addOption("sonar","Sonar (Standard)").addOption("sonar-deep-research","Sonar Deep Research (Exhaustive)").addOption("sonar-reasoning","Sonar Reasoning (Fast)").setValue(this.plugin.settings.perplexityModel||"sonar-pro").onChange(async v=>{this.plugin.settings.perplexityModel=v,Object.keys(this.plugin.settings.researchModeConfigs).forEach(x=>{this.plugin.settings.researchModeConfigs[x].perplexityModel&&(this.plugin.settings.researchModeConfigs[x].perplexityModel=v)}),await this.plugin.saveSettings()})});let i=e.createEl("div",{cls:"setting-item"}),t=i.createEl("div",{cls:"setting-item-info"});t.createEl("div",{cls:"setting-item-name",text:"Temperature"});let s=t.createEl("div",{cls:"setting-item-description"}),r=this.plugin.settings.perplexityTemperature||.2;s.textContent=`Creativity level (0.0-2.0). Current: ${r}`,i.createEl("div",{cls:"setting-item-control"}).createEl("input",{type:"range",cls:"slider",attr:{min:"0",max:"2.0",step:"0.1",value:r.toString()}}).addEventListener("input",async b=>{let v=parseFloat(b.target.value);this.plugin.settings.perplexityTemperature=v,s.textContent=`Creativity level (0.0-2.0). Current: ${v}`,Object.keys(this.plugin.settings.researchModeConfigs).forEach(x=>{this.plugin.settings.researchModeConfigs[x].perplexityTemperature=v}),await this.plugin.saveSettings()});let n=e.createEl("div",{cls:"setting-item"}),a=n.createEl("div",{cls:"setting-item-info"});a.createEl("div",{cls:"setting-item-name",text:"Max Tokens"});let l=a.createEl("div",{cls:"setting-item-description"}),d=this.plugin.settings.perplexityMaxTokens||1024;l.textContent=`Maximum response length (256-4096). Current: ${d}`,n.createEl("div",{cls:"setting-item-control"}).createEl("input",{type:"range",cls:"slider",attr:{min:"256",max:"4096",step:"64",value:d.toString()}}).addEventListener("input",async b=>{let v=parseInt(b.target.value);this.plugin.settings.perplexityMaxTokens=v,l.textContent=`Maximum response length (256-4096). Current: ${v}`,Object.keys(this.plugin.settings.researchModeConfigs).forEach(x=>{this.plugin.settings.researchModeConfigs[x].perplexityMaxTokens=v}),await this.plugin.saveSettings()});let f=e.createEl("div",{cls:"setting-item"}),g=f.createEl("div",{cls:"setting-item-info"});g.createEl("div",{cls:"setting-item-name",text:"Top P"});let y=g.createEl("div",{cls:"setting-item-description"}),C=this.plugin.settings.perplexityTopP||1;y.textContent=`Nucleus sampling (0.1-1.0). Current: ${C}`,f.createEl("div",{cls:"setting-item-control"}).createEl("input",{type:"range",cls:"slider",attr:{min:"0.1",max:"1.0",step:"0.05",value:C.toString()}}).addEventListener("input",async b=>{let v=parseFloat(b.target.value);this.plugin.settings.perplexityTopP=v,y.textContent=`Nucleus sampling (0.1-1.0). Current: ${v}`,Object.keys(this.plugin.settings.researchModeConfigs).forEach(x=>{this.plugin.settings.researchModeConfigs[x].perplexityTopP=v}),await this.plugin.saveSettings()});let A=e.createEl("div",{cls:"setting-item"}),D=A.createEl("div",{cls:"setting-item-info"});D.createEl("div",{cls:"setting-item-name",text:"Top K"});let R=D.createEl("div",{cls:"setting-item-description"}),V=this.plugin.settings.perplexityTopK||0;R.textContent=`Vocabulary limitation (0-100). Current: ${V}`,A.createEl("div",{cls:"setting-item-control"}).createEl("input",{type:"range",cls:"slider",attr:{min:"0",max:"100",step:"5",value:V.toString()}}).addEventListener("input",async b=>{let v=parseInt(b.target.value);this.plugin.settings.perplexityTopK=v,R.textContent=`Vocabulary limitation (0-100, 0=disabled). Current: ${v}`,Object.keys(this.plugin.settings.researchModeConfigs).forEach(x=>{this.plugin.settings.researchModeConfigs[x].perplexityTopK=v}),await this.plugin.saveSettings()});let q=e.createEl("div",{cls:"setting-item"}),F=q.createEl("div",{cls:"setting-item-info"});F.createEl("div",{cls:"setting-item-name",text:"Frequency Penalty"});let $=F.createEl("div",{cls:"setting-item-description"}),H=this.plugin.settings.perplexityFrequencyPenalty||0;$.textContent=`Reduce repetition (-2.0 to 2.0). Current: ${H}`,q.createEl("div",{cls:"setting-item-control"}).createEl("input",{type:"range",cls:"slider",attr:{min:"-2.0",max:"2.0",step:"0.1",value:H.toString()}}).addEventListener("input",async b=>{let v=parseFloat(b.target.value);this.plugin.settings.perplexityFrequencyPenalty=v,$.textContent=`Reduce repetition (-2.0 to 2.0). Current: ${v}`,Object.keys(this.plugin.settings.researchModeConfigs).forEach(x=>{this.plugin.settings.researchModeConfigs[x].perplexityFrequencyPenalty=v}),await this.plugin.saveSettings()});let z=e.createEl("div",{cls:"setting-item"}),U=z.createEl("div",{cls:"setting-item-info"});U.createEl("div",{cls:"setting-item-name",text:"Presence Penalty"});let W=U.createEl("div",{cls:"setting-item-description"}),G=this.plugin.settings.perplexityPresencePenalty||0;W.textContent=`Encourage new topics (-2.0 to 2.0). Current: ${G}`,z.createEl("div",{cls:"setting-item-control"}).createEl("input",{type:"range",cls:"slider",attr:{min:"-2.0",max:"2.0",step:"0.1",value:G.toString()}}).addEventListener("input",async b=>{let v=parseFloat(b.target.value);this.plugin.settings.perplexityPresencePenalty=v,W.textContent=`Encourage new topics (-2.0 to 2.0). Current: ${v}`,Object.keys(this.plugin.settings.researchModeConfigs).forEach(x=>{this.plugin.settings.researchModeConfigs[x].perplexityPresencePenalty=v}),await this.plugin.saveSettings()}),new p.Setting(e).setName("Return Citations").setDesc("Include source citations in responses").addToggle(b=>{var v;b.setValue((v=this.plugin.settings.perplexityReturnCitations)!=null?v:!0).onChange(async x=>{this.plugin.settings.perplexityReturnCitations=x,Object.keys(this.plugin.settings.researchModeConfigs).forEach(I=>{this.plugin.settings.researchModeConfigs[I].perplexityReturnCitations=x}),await this.plugin.saveSettings()})}),new p.Setting(e).setName("Return Images").setDesc("Include images in search results when available").addToggle(b=>{var v;b.setValue((v=this.plugin.settings.perplexityReturnImages)!=null?v:!1).onChange(async x=>{this.plugin.settings.perplexityReturnImages=x,Object.keys(this.plugin.settings.researchModeConfigs).forEach(I=>{this.plugin.settings.researchModeConfigs[I].perplexityReturnImages=x}),await this.plugin.saveSettings()})}),new p.Setting(e).setName("Return Related Questions").setDesc("Include suggested follow-up questions").addToggle(b=>{var v;b.setValue((v=this.plugin.settings.perplexityReturnRelated)!=null?v:!0).onChange(async x=>{this.plugin.settings.perplexityReturnRelated=x,Object.keys(this.plugin.settings.researchModeConfigs).forEach(I=>{this.plugin.settings.researchModeConfigs[I].perplexityReturnRelated=x}),await this.plugin.saveSettings()})}),new p.Setting(e).setName("Search Domain Time").setDesc("Time range for search results").addDropdown(b=>{b.addOption("","No time limit").addOption("hour","Past hour").addOption("day","Past day").addOption("week","Past week").addOption("month","Past month").addOption("year","Past year").setValue(this.plugin.settings.perplexitySearchDomainFilter||"").onChange(async v=>{this.plugin.settings.perplexitySearchDomainFilter=v,Object.keys(this.plugin.settings.researchModeConfigs).forEach(x=>{this.plugin.settings.researchModeConfigs[x].perplexitySearchDomainFilter=v}),await this.plugin.saveSettings()})})}addTavilyAdvancedSettings(e){e.createEl("h5",{text:"Tavily Advanced Parameters"}),e.createEl("p",{text:"Configure Tavily search depth and result filtering options.",cls:"settings-help-text"}),new p.Setting(e).setName("Search Depth").setDesc("Depth of search results to retrieve").addDropdown(f=>{f.addOption("basic","Basic - Fast results").addOption("advanced","Advanced - Comprehensive results").setValue(this.plugin.settings.tavilySearchDepth||"basic").onChange(async g=>{this.plugin.settings.tavilySearchDepth=g,Object.keys(this.plugin.settings.researchModeConfigs).forEach(y=>{this.plugin.settings.researchModeConfigs[y].tavilySearchDepth=g}),await this.plugin.saveSettings()})});let i=e.createEl("div",{cls:"setting-item"}),t=i.createEl("div",{cls:"setting-item-info"});t.createEl("div",{cls:"setting-item-name",text:"Max Results"});let s=t.createEl("div",{cls:"setting-item-description"}),r=this.plugin.settings.tavilyMaxResults||5;s.textContent=`Maximum number of results (1-20). Current: ${r}`,i.createEl("div",{cls:"setting-item-control"}).createEl("input",{type:"range",cls:"slider",attr:{min:"1",max:"20",step:"1",value:r.toString()}}).addEventListener("input",async f=>{let g=parseInt(f.target.value);this.plugin.settings.tavilyMaxResults=g,s.textContent=`Maximum number of results (1-20). Current: ${g}`,Object.keys(this.plugin.settings.researchModeConfigs).forEach(y=>{this.plugin.settings.researchModeConfigs[y].tavilyMaxResults=g}),await this.plugin.saveSettings()}),new p.Setting(e).setName("Include Domains").setDesc("Comma-separated list of domains to include (e.g., wikipedia.org, reddit.com)").addTextArea(f=>{f.setPlaceholder("Enter domains to include...").setValue(this.plugin.settings.tavilyIncludeDomains||"").onChange(async g=>{this.plugin.settings.tavilyIncludeDomains=g,Object.keys(this.plugin.settings.researchModeConfigs).forEach(y=>{this.plugin.settings.researchModeConfigs[y].tavilyIncludeDomains=g}),await this.plugin.saveSettings()}),f.inputEl.rows=2}),new p.Setting(e).setName("Exclude Domains").setDesc("Comma-separated list of domains to exclude (e.g., ads.com, spam.com)").addTextArea(f=>{f.setPlaceholder("Enter domains to exclude...").setValue(this.plugin.settings.tavilyExcludeDomains||"").onChange(async g=>{this.plugin.settings.tavilyExcludeDomains=g,Object.keys(this.plugin.settings.researchModeConfigs).forEach(y=>{this.plugin.settings.researchModeConfigs[y].tavilyExcludeDomains=g}),await this.plugin.saveSettings()}),f.inputEl.rows=2}),new p.Setting(e).setName("Include Answer").setDesc("Include a direct answer in addition to search results").addToggle(f=>{var g;f.setValue((g=this.plugin.settings.tavilyIncludeAnswer)!=null?g:!0).onChange(async y=>{this.plugin.settings.tavilyIncludeAnswer=y,Object.keys(this.plugin.settings.researchModeConfigs).forEach(C=>{this.plugin.settings.researchModeConfigs[C].tavilyIncludeAnswer=y}),await this.plugin.saveSettings()})}),new p.Setting(e).setName("Include Raw Content").setDesc("Include the raw HTML content of search results").addToggle(f=>{var g;f.setValue((g=this.plugin.settings.tavilyIncludeRawContent)!=null?g:!1).onChange(async y=>{this.plugin.settings.tavilyIncludeRawContent=y,Object.keys(this.plugin.settings.researchModeConfigs).forEach(C=>{this.plugin.settings.researchModeConfigs[C].tavilyIncludeRawContent=y}),await this.plugin.saveSettings()})}),new p.Setting(e).setName("Include Images").setDesc("Include images in search results when available").addToggle(f=>{var g;f.setValue((g=this.plugin.settings.tavilyIncludeImages)!=null?g:!1).onChange(async y=>{this.plugin.settings.tavilyIncludeImages=y,Object.keys(this.plugin.settings.researchModeConfigs).forEach(C=>{this.plugin.settings.researchModeConfigs[C].tavilyIncludeImages=y}),await this.plugin.saveSettings()})}),new p.Setting(e).setName("Topic").setDesc("Topic category for search context (general, news, etc.)").addDropdown(f=>{f.addOption("general","General").addOption("news","News").addOption("science","Science").addOption("technology","Technology").addOption("business","Business").addOption("health","Health").addOption("entertainment","Entertainment").addOption("sports","Sports").setValue(this.plugin.settings.tavilyTopic||"general").onChange(async g=>{this.plugin.settings.tavilyTopic=g,Object.keys(this.plugin.settings.researchModeConfigs).forEach(y=>{this.plugin.settings.researchModeConfigs[y].tavilyTopic=g}),await this.plugin.saveSettings()})});let n=e.createEl("div",{cls:"setting-item"}),a=n.createEl("div",{cls:"setting-item-info"});a.createEl("div",{cls:"setting-item-name",text:"Recent Content Days"});let l=a.createEl("div",{cls:"setting-item-description"}),d=this.plugin.settings.tavilyDays||0;l.textContent=`Limit to content from last N days (0=no limit). Current: ${d}`,n.createEl("div",{cls:"setting-item-control"}).createEl("input",{type:"range",cls:"slider",attr:{min:"0",max:"365",step:"1",value:d.toString()}}).addEventListener("input",async f=>{let g=parseInt(f.target.value);this.plugin.settings.tavilyDays=g,l.textContent=`Limit to content from last N days (0=no limit). Current: ${g}`,Object.keys(this.plugin.settings.researchModeConfigs).forEach(y=>{this.plugin.settings.researchModeConfigs[y].tavilyDays=g}),await this.plugin.saveSettings()})}addExaAdvancedSettings(e){e.createEl("h5",{text:"Exa Neural Search Advanced Parameters"}),e.createEl("p",{text:"Exa offers the most customization options for semantic search and content filtering.",cls:"settings-help-text"}),new p.Setting(e).setName("Search Type").setDesc("Type of search to perform").addDropdown(n=>{n.addOption("neural","Neural - Semantic understanding").addOption("keyword","Keyword - Traditional search").addOption("auto","Auto - Best of both").setValue(this.plugin.settings.exaSearchType||"neural").onChange(async a=>{this.plugin.settings.exaSearchType=a,Object.keys(this.plugin.settings.researchModeConfigs).forEach(l=>{this.plugin.settings.researchModeConfigs[l].exaSearchType=a}),await this.plugin.saveSettings()})}),new p.Setting(e).setName("Use Autoprompt").setDesc("Automatically enhance queries for better results").addToggle(n=>{var a;n.setValue((a=this.plugin.settings.exaUseAutoprompt)!=null?a:!0).onChange(async l=>{this.plugin.settings.exaUseAutoprompt=l,Object.keys(this.plugin.settings.researchModeConfigs).forEach(d=>{this.plugin.settings.researchModeConfigs[d].exaUseAutoprompt=l}),await this.plugin.saveSettings()})}),new p.Setting(e).setName("Category").setDesc("Content category to focus search on").addDropdown(n=>{n.addOption("","Any category").addOption("company","Company").addOption("research paper","Research Paper").addOption("news","News").addOption("github","GitHub").addOption("tweet","Tweet").addOption("movie","Movie").addOption("song","Song").addOption("personal site","Personal Site").addOption("pdf","PDF Document").setValue(this.plugin.settings.exaCategory||"").onChange(async a=>{this.plugin.settings.exaCategory=a,Object.keys(this.plugin.settings.researchModeConfigs).forEach(l=>{this.plugin.settings.researchModeConfigs[l].exaCategory=a}),await this.plugin.saveSettings()})});let i=e.createEl("div",{cls:"setting-item"}),t=i.createEl("div",{cls:"setting-item-info"});t.createEl("div",{cls:"setting-item-name",text:"Number of Results"});let s=t.createEl("div",{cls:"setting-item-description"}),r=this.plugin.settings.exaNumResults||10;s.textContent=`Number of results to return (1-20). Current: ${r}`,i.createEl("div",{cls:"setting-item-control"}).createEl("input",{type:"range",cls:"slider",attr:{min:"1",max:"20",step:"1",value:r.toString()}}).addEventListener("input",async n=>{let a=parseInt(n.target.value);this.plugin.settings.exaNumResults=a,s.textContent=`Number of results to return (1-20). Current: ${a}`,Object.keys(this.plugin.settings.researchModeConfigs).forEach(l=>{this.plugin.settings.researchModeConfigs[l].exaNumResults=a}),await this.plugin.saveSettings()}),new p.Setting(e).setName("Include Domains").setDesc("Comma-separated list of domains to include (e.g., wikipedia.org, arxiv.org)").addTextArea(n=>{n.setPlaceholder("Enter domains to include...").setValue(this.plugin.settings.exaIncludeDomains||"").onChange(async a=>{this.plugin.settings.exaIncludeDomains=a,Object.keys(this.plugin.settings.researchModeConfigs).forEach(l=>{this.plugin.settings.researchModeConfigs[l].exaIncludeDomains=a}),await this.plugin.saveSettings()}),n.inputEl.rows=2}),new p.Setting(e).setName("Exclude Domains").setDesc("Comma-separated list of domains to exclude (e.g., pinterest.com, quora.com)").addTextArea(n=>{n.setPlaceholder("Enter domains to exclude...").setValue(this.plugin.settings.exaExcludeDomains||"").onChange(async a=>{this.plugin.settings.exaExcludeDomains=a,Object.keys(this.plugin.settings.researchModeConfigs).forEach(l=>{this.plugin.settings.researchModeConfigs[l].exaExcludeDomains=a}),await this.plugin.saveSettings()}),n.inputEl.rows=2}),new p.Setting(e).setName("Start Crawl Date").setDesc("Only include results crawled after this date (YYYY-MM-DD format)").addText(n=>{n.setPlaceholder("YYYY-MM-DD").setValue(this.plugin.settings.exaStartCrawlDate||"").onChange(async a=>{this.plugin.settings.exaStartCrawlDate=a,Object.keys(this.plugin.settings.researchModeConfigs).forEach(l=>{this.plugin.settings.researchModeConfigs[l].exaStartCrawlDate=a}),await this.plugin.saveSettings()})}),new p.Setting(e).setName("End Crawl Date").setDesc("Only include results crawled before this date (YYYY-MM-DD format)").addText(n=>{n.setPlaceholder("YYYY-MM-DD").setValue(this.plugin.settings.exaEndCrawlDate||"").onChange(async a=>{this.plugin.settings.exaEndCrawlDate=a,Object.keys(this.plugin.settings.researchModeConfigs).forEach(l=>{this.plugin.settings.researchModeConfigs[l].exaEndCrawlDate=a}),await this.plugin.saveSettings()})}),new p.Setting(e).setName("Start Published Date").setDesc("Only include results published after this date (YYYY-MM-DD format)").addText(n=>{n.setPlaceholder("YYYY-MM-DD").setValue(this.plugin.settings.exaStartPublishedDate||"").onChange(async a=>{this.plugin.settings.exaStartPublishedDate=a,Object.keys(this.plugin.settings.researchModeConfigs).forEach(l=>{this.plugin.settings.researchModeConfigs[l].exaStartPublishedDate=a}),await this.plugin.saveSettings()})}),new p.Setting(e).setName("End Published Date").setDesc("Only include results published before this date (YYYY-MM-DD format)").addText(n=>{n.setPlaceholder("YYYY-MM-DD").setValue(this.plugin.settings.exaEndPublishedDate||"").onChange(async a=>{this.plugin.settings.exaEndPublishedDate=a,Object.keys(this.plugin.settings.researchModeConfigs).forEach(l=>{this.plugin.settings.researchModeConfigs[l].exaEndPublishedDate=a}),await this.plugin.saveSettings()})}),new p.Setting(e).setName("Include Text").setDesc("Include the text content of search results").addToggle(n=>{var a;n.setValue((a=this.plugin.settings.exaIncludeText)!=null?a:!0).onChange(async l=>{this.plugin.settings.exaIncludeText=l,Object.keys(this.plugin.settings.researchModeConfigs).forEach(d=>{this.plugin.settings.researchModeConfigs[d].exaIncludeText=l}),await this.plugin.saveSettings()})}),new p.Setting(e).setName("Include Highlights").setDesc("Include highlighted relevant excerpts from results").addToggle(n=>{var a;n.setValue((a=this.plugin.settings.exaIncludeHighlights)!=null?a:!0).onChange(async l=>{this.plugin.settings.exaIncludeHighlights=l,Object.keys(this.plugin.settings.researchModeConfigs).forEach(d=>{this.plugin.settings.researchModeConfigs[d].exaIncludeHighlights=l}),await this.plugin.saveSettings()})})}addCustomPromptsSettings(e){let i=e.createDiv("custom-prompts-section");i.createEl("h4",{text:"Custom Prompt Templates"});let t=i.createEl("p",{cls:"settings-help-text"});t.innerHTML=`
			\u{1F3AF} <strong>Progressive Disclosure Design:</strong><br>
			...
			\u{1F4A1} All prompts support Obsidian-compatible markdown formatting and citations.
		`,new p.Setting(i).setName("Prompt Template").setDesc("Choose a preset template or create custom prompts").addDropdown(l=>{l.addOption("built-in","\u{1F3E0} Built-in Optimized (Recommended)").addOption("academic","\u{1F393} Academic Research Template").addOption("business","\u{1F4BC} Business Analysis Template").addOption("creative","\u{1F3A8} Creative Writing Template").addOption("technical","\u2699\uFE0F Technical Documentation Template").addOption("custom","\u{1F6E0}\uFE0F Custom (Manual Configuration)").setValue("built-in").onChange(async d=>{d!=="custom"&&d!=="built-in"&&confirm(`Load ${d} template? This will overwrite any existing custom prompts.`)&&(await this.loadPromptTemplate(d),this.display())})}),this.addPromptEditor(i,"Quick Mode","quickPrompt","Fast, concise responses (250-350 words, 2-3 key points)"),this.addPromptEditor(i,"Comprehensive Mode","comprehensivePrompt","Balanced analysis (600-800 words, structured approach)"),this.addPromptEditor(i,"Deep Mode","deepPrompt","In-depth research (1000-1500 words, comprehensive analysis)"),this.addPromptEditor(i,"Reasoning Mode","reasoningPrompt","Logical analysis with evidence evaluation and bias detection"),this.addPromptEditor(i,"YouTube Mode","youtubePrompt","Video content analysis with timestamps and citations");let s=i.createDiv("prompt-preview-section");s.createEl("h5",{text:"Prompt Validation & Management"});let r=s.createEl("div",{cls:"prompt-buttons"});r.createEl("button",{text:"\u{1F50D} Validate All Prompts",cls:"mod-cta"}).addEventListener("click",()=>{this.validateAllPrompts(s)}),r.createEl("button",{text:"\u{1F504} Reset to Defaults",cls:"mod-warning"}).addEventListener("click",async()=>{confirm("Reset all custom prompts to optimized default values? This cannot be undone.")&&(await this.resetPromptsToDefaults(),this.display())}),r.createEl("button",{text:"\u{1F4E4} Export Prompts",cls:"prompt-export-btn"}).addEventListener("click",()=>{this.exportPrompts()}),r.createEl("button",{text:"\u{1F4E5} Import Prompts",cls:"prompt-import-btn"}).addEventListener("click",()=>{this.importPrompts()})}addPromptEditor(e,i,t,s){let r=new p.Setting(e).setName(i+" Prompt").setDesc(`${s}. Use {query} as placeholder for user input.`).addTextArea(l=>{l.setPlaceholder("Enter your custom prompt...").setValue(this.plugin.settings[t]).onChange(async d=>{d.includes("{query}")||new p.Notice("\u26A0\uFE0F Warning: Prompt should include {query} placeholder for user input"),d.length>5e3&&new p.Notice("\u26A0\uFE0F Warning: Prompt is very long and may cause API errors"),this.plugin.settings[t]=d,await this.plugin.saveSettings()}),l.inputEl.rows=8,l.inputEl.style.fontFamily="var(--font-monospace)",l.inputEl.style.fontSize="var(--font-ui-small)"}),c=e.createEl("div",{cls:"prompt-controls"}),o=c.createEl("span",{cls:"prompt-char-count",text:`${(this.plugin.settings[t]||"").length} characters`});c.createEl("button",{text:"\u{1F441}\uFE0F Preview",cls:"prompt-preview-btn"}).addEventListener("click",()=>{this.previewPrompt(t,i)});let a=r.settingEl.querySelector("textarea");a&&a.addEventListener("input",()=>{o.textContent=`${a.value.length} characters`})}async loadPromptTemplate(e){if(e==="built-in")return;let t={academic:{quick:`### \u{1F393} Academic Quick Analysis
...
**Standards**: 250-300 words, minimum 4 academic citations, peer-reviewed focus`,comprehensive:`### \u{1F4DA} Academic Research Framework
...
**Standards**: 600-800 words, 13+ peer-reviewed citations, systematic review approach`,deep:`### \u{1F52C} Comprehensive Academic Investigation
...
**Standards**: 1000-1500 words, 23+ peer-reviewed citations, comprehensive academic analysis`,reasoning:`### \u{1F9E0} Academic Critical Reasoning
...
**Standards**: 800-1000 words, 22+ citations, systematic logical analysis`,youtube:`### \u{1F3AC} Academic Video Content Analysis
...
**Standards**: Academic rigor, proper citation, fact-checking focus`},business:{quick:`### \u{1F4BC} Business Quick Insight
...
**Focus**: ROI, market impact, actionable insights`,comprehensive:`### \u{1F4CA} Business Analysis Framework
...
**Standards**: 500-700 words, focus on ROI and implementation`,deep:`### \u{1F3AF} Strategic Business Deep Dive
...
**Standards**: 900-1200 words, comprehensive business strategy focus`,reasoning:`### \u{1F9EE} Business Logic & Decision Framework
...
**Standards**: Data-driven decision making, logical business case`,youtube:`### \u{1F4F9} Business Video Intelligence
...
**Standards**: Business-focused analysis, ROI orientation, implementation ready`},creative:{quick:`### \u2728 Creative Quick Spark
...
**Focus**: Inspiration, originality, emotional impact`,comprehensive:`### \u{1F3A8} Creative Development Framework
...
**Standards**: 500-700 words, focus on originality and emotional impact`,deep:`### \u{1F308} Creative Deep Exploration
...
**Standards**: 800-1200 words, emphasis on artistic innovation and cultural depth`,reasoning:`### \u{1F3AD} Creative Logic & Artistic Reasoning
...
**Standards**: Logical creative analysis, evidence-based artistic decisions`,youtube:`### \u{1F3AC} Creative Video Analysis
...
**Standards**: Creative focus, artistic development, inspiration-driven analysis`},technical:{quick:`### \u26A1 Technical Quick Reference
...
**Focus**: Technical accuracy, implementation details, practical application`,comprehensive:`### \u{1F527} Technical Documentation Framework
...
**Standards**: 500-700 words, focus on implementation and technical accuracy`,deep:`### \u{1F3D7}\uFE0F Technical Deep Architecture
...
**Standards**: 900-1200 words, comprehensive technical analysis`,reasoning:`### \u{1F9E0} Technical Logic & System Reasoning
...
**Standards**: Logical technical analysis, evidence-based engineering decisions`,youtube:`### \u{1F6E0}\uFE0F Technical Video Analysis
...
**Standards**: Technical accuracy, implementation focus, engineering best practices`}}[e];t&&(this.plugin.settings.quickPrompt=t.quick,this.plugin.settings.comprehensivePrompt=t.comprehensive,this.plugin.settings.deepPrompt=t.deep,this.plugin.settings.reasoningPrompt=t.reasoning,this.plugin.settings.youtubePrompt=t.youtube,await this.plugin.saveSettings(),new p.Notice(`\u2705 Loaded ${e} prompt template successfully`))}previewPrompt(e,i){let t=this.plugin.settings[e],s=t.replace("{query}","example search query"),r=new p.Modal(this.app);r.titleEl.textContent=`Preview: ${i} Prompt`;let c=r.contentEl;c.createEl("h3",{text:"Prompt Preview"}),c.createEl("p",{text:"This is how your prompt will appear with a sample query:"});let o=c.createEl("div",{cls:"prompt-preview-content"});o.style.cssText=`
			background: var(--background-secondary);
			padding: var(--size-4-2);
			border-radius: var(--radius-s);
			font-family: var(--font-monospace);
			...
			overflow-y: auto;
			border: 1px solid var(--background-modifier-border);
		`,o.textContent=s;let n=c.createEl("div",{cls:"prompt-info"});if(n.createEl("p",{text:`Character count: ${t.length}`}),!t.includes("{query}")){let a=n.createEl("p",{text:"\u26A0\uFE0F Warning: Prompt does not contain {query} placeholder",cls:"prompt-warning"});a.style.color="var(--text-error)"}if(t.length>5e3){let a=n.createEl("p",{text:"\u26A0\uFE0F Warning: Prompt is very long and may cause API errors",cls:"prompt-warning"});a.style.color="var(--text-error)"}r.open()}validateAllPrompts(e){let i=e.createEl("div",{cls:"validation-results"});i.empty();let t=[{key:"quickPrompt",name:"Quick Mode",maxLength:2e3},{key:"comprehensivePrompt",name:"Comprehensive Mode",maxLength:4e3},{key:"deepPrompt",name:"Deep Mode",maxLength:6e3},{key:"reasoningPrompt",name:"Reasoning Mode",maxLength:5e3},{key:"youtubePrompt",name:"YouTube Mode",maxLength:4e3}],s=!0,r=0;t.forEach(n=>{let a=this.plugin.settings[n.key],l=i.createEl("div",{cls:"validation-item"});!a||a.trim().length===0?(l.innerHTML=`\u274C ${n.name}: Empty prompt`,l.style.color="var(--text-error)",s=!1):a.includes("{query}")?a.length>n.maxLength?(l.innerHTML=`\u26A0\uFE0F ${n.name}: Too long (${a.length}/${n.maxLength} chars)`,l.style.color="var(--text-warning)"):a.length>5e3?(l.innerHTML=`\u26A0\uFE0F ${n.name}: Very long (${a.length} chars) - may cause API errors`,l.style.color="var(--text-warning)"):(l.innerHTML=`\u2705 ${n.name}: Valid (${a.length} chars)`,l.style.color="var(--text-success)",r++):(l.innerHTML=`\u26A0\uFE0F ${n.name}: Missing {query} placeholder`,l.style.color="var(--text-warning)",s=!1)});let c=i.createEl("div",{cls:"validation-summary"});s&&r===t.length?(c.innerHTML="\u{1F389} All prompts are valid and ready to use!",c.style.color="var(--text-success)",c.style.fontWeight="var(--font-weight-bold)"):(c.innerHTML=`\u{1F4CA} Validation Summary: ${r}/${t.length} prompts are fully valid`,c.style.color="var(--text-muted)");let o=i.createEl("div",{cls:"validation-tips"});o.innerHTML=`
			<h6>\u{1F4A1} Prompt Optimization Tips:</h6>
			<ul>
		...
		`}async resetPromptsToDefaults(){this.plugin.settings.quickPrompt=O.quickPrompt,this.plugin.settings.comprehensivePrompt=O.comprehensivePrompt,this.plugin.settings.deepPrompt=O.deepPrompt,this.plugin.settings.reasoningPrompt=O.reasoningPrompt,this.plugin.settings.youtubePrompt=O.youtubePrompt,await this.plugin.saveSettings(),new p.Notice("\u2705 All prompts reset to default values")}exportPrompts(){let e={quickPrompt:this.plugin.settings.quickPrompt,comprehensivePrompt:this.plugin.settings.comprehensivePrompt,deepPrompt:this.plugin.settings.deepPrompt,reasoningPrompt:this.plugin.settings.reasoningPrompt,youtubePrompt:this.plugin.settings.youtubePrompt,exportDate:new Date().toISOString(),pluginVersion:this.plugin.manifest.version},i=JSON.stringify(e,null,2),t=new Blob([i],{type:"application/json"}),s=document.createElement("a");s.href=URL.createObjectURL(t),s.download=`ai-web-search-custom-prompts-${new Date().toISOString().split("T")[0]}.json`,s.click(),new p.Notice("\u{1F4E4} Custom prompts exported successfully")}importPrompts(){let e=document.createElement("input");e.type="file",e.accept=".json",e.onchange=async i=>{var s;let t=(s=i.target.files)==null?void 0:s[0];if(t)try{let r=await t.text(),c=JSON.parse(r),o=["quickPrompt","comprehensivePrompt","deepPrompt","reasoningPrompt","youtubePrompt"],n=o.filter(l=>!c[l]);if(n.length>0){new p.Notice(`\u274C Invalid prompt file: Missing fields ${n.join(", ")}`);return}let a=o.filter(l=>!c[l].includes("{query}"));if(a.length>0&&!confirm(`\u26A0\uFE0F Some prompts are missing {query} placeholder: ${a.join(", ")}. Import anyway?`))return;this.plugin.settings.quickPrompt=c.quickPrompt,this.plugin.settings.comprehensivePrompt=c.comprehensivePrompt,this.plugin.settings.deepPrompt=c.deepPrompt,this.plugin.settings.reasoningPrompt=c.reasoningPrompt,this.plugin.settings.youtubePrompt=c.youtubePrompt,await this.plugin.saveSettings(),this.display(),new p.Notice("\u{1F4E5} Custom prompts imported successfully")}catch(r){new p.Notice("\u274C Failed to import prompts: Invalid file format"),console.error("Import error:",r)}},e.click()}};
