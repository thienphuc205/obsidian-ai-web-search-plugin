	...
	async handleSend(message: string, insertToNote: boolean, saveToFolder: boolean = false) {
		...
		try {
			...
			if (this.currentResearchMode?.id === 'youtube' && this.currentVideoContext) {
				...
				if (provider === 'gemini') {
					response = await this.plugin.performGeminiChat(message, chatHistory);
				} else if (provider === 'perplexity') {
					response = await this.plugin.performPerplexityChat(message, chatHistory);
				} else {
...
`;
		...
		styleEl.id = 'ai-web-search-settings-styles';
		styleEl.textContent = cssText;
		
		// Remove existing styles if any
		const existingStyle = document.getElementById('ai-web-search-settings-styles');
		if (existingStyle) {
	...
	async performGeminiChat(message: string, chatHistory: Array<{role: string, content: string}>): Promise<string> {
		return await this.chatWithGemini(message, chatHistory);
	}

	async performPerplexityChat(message: string, chatHistory: Array<{role: string, content: string}>): Promise<string> {
		...
		try {
			...
			throw new Error(`Perplexity chat failed: ${error instanceof Error ? error.message : String(error)}`);
		}
	}
